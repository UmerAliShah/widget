...
const WidgetPopup = () => {
  ...
  const handleAmountClick = (org, amount) => {
    ...
    const existingDonationIndex = donationData.findIndex(item => item.organization === org.name);
  ...
  const handleDonation = () => {
    ...
    // ...
    localStorage.setItem("donationData", JSON.stringify([...donationData, donationData]));
          ...
          <div>
            <Slider {...settings}>
              {organizations.map((org, index) => {
                const isAmountSelected = selectedAmount > 0 && org.name === donationData.find(item => item.organization === org.name)?.organization;
                return (
                  <div className="main-slide" key={index}>
                    ...
                    <div className="Actions">
                      <Button
                        className={`btn-1 ${selectedAmount === 10 ? 'active' : ''}`}
                        onClick={() => handleAmountClick(org, 10)}
                      >
                        10
                      </Button>
                      <Button
                        className={`btn-2 ${selectedAmount === 20 ? 'active' : ''}`}
                        onClick={() => handleAmountClick(org, 20)}
                      >
                        20
                      </Button>
                      <Button
                        className={`btn-3 ${selectedAmount === 30 ? 'active' : ''}`}
